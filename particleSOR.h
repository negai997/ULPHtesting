...
namespace sph {
	...
	{
		//unsigned int* idx;
		//idx = (unsigned int*)malloc(idp * sizeof(unsigned int));
		cudaMallocManaged()
		//double* x;
		x = (double*)malloc(idp * sizeof(double));
		double* y;
		double* ux;
		double* uy;
		double* vx;
	...
	inline void particleSOA::setvolume(double _v)
	{
		vol = _v;
	}

	inline void particleSOA::setdensity(double _d)
	{
		rho = _d;
	}

	inline void particleSOA::setInitPressure(double _p)
	{
		press0 = _p;
	}

	inline void particleSOA::setInitSoundSpd(double _c0)
	{
		c0 = _c0;
	}

	inline void particleSOA::setVisco(double _v)
	{
		visco = _v;
	}

	inline void particleSOA::sethsml(double _hsml)
	{
		hsml = _hsml;
	}

	inline void particleSOA::setBtype(BoundaryType _b)
	{
		btype = _b;
	}

	inline void particleSOA::setFtype(FixType _f)
	{
		ftype = _f;
	}

	inline void particleSOA::setFltype(FluidType _f)
	{
		fltype = _f;
	}

	inline void particleSOA::setIotype(InoutType _i)
	{
		iotype = _i;
	}

	inline void particleSOA::setDensityMin()
	{
		rho_min = rho0 - back_p / c0 / c0;
	}

	inline const double particleSOA::getdt()
	{
	...
	inline void particleSOA::storeHalf()
	{
		half_x = x;
		half_y = y;
		half_vx = vx;
	...
	inline void particleSOA::density_filter()
	{
		double beta0_mls = 0;
	...
	inline void particleSOA::updatePressure()
	{
	...
	inline void particleSOA::updateDDensity()
	{
		// diffusion term
	...
	inline void particleSOA::updateFint()
	{
		fintx = finty = 0;
		const double hsml = this->gethsml();
	...
	inline void particleSOA::getTurbulence()
	{
		math::matrix dudx(2);
		math::matrix unit(2);
	...
	inline void particleSOA::getTurbForce()
	{
		turbx = turby = 0;
	...
	inline void particleSOA::getArticial()
	{
		avx = avy = 0;
	...
	inline void particleSOA::updateAcc()
	{
		ax = fintx + avx + turbx + replx;
		ay = finty + avy + turby + reply;
	}

	inline void particleSOA::integration1sthalf(const double _dt2)
	{
		if (this->getBtype() == BoundaryType::Boundary) return;
	...
	inline void particleSOA::integrationfull(const double _dt)
	{
		if (this->getBtype() == BoundaryType::Boundary) return;
	...
	inline void particleSOA::shifting_c()
	{
		shift_c = 0;
		if (this->getBtype() == BoundaryType::Boundary) return;
	...
	inline void particleSOA::shifting(const double dt)
	{
		shift_x = shift_y = 0;
